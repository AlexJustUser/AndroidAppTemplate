...
fun ChartScreen(
    ...
    activity.bluetoothHelper.onDeviceResult = {
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.temperature).plus(SPACE_STRING)
                .plus(state.dataLYWSD03MMC.temperature.toString())
        )
        Text(
            text = stringResource(R.string.humidity).plus(SPACE_STRING)
                .plus(state.dataLYWSD03MMC.humidity.toString())
        )
        Text(
            text = stringResource(R.string.battery).plus(SPACE_STRING)
                .plus(state.dataLYWSD03MMC.battery.toString())
        )
        AndroidView(
            modifier = Modifier
                .fillMaxSize()
